...
1. Azure AI 파운드리 포털의 탐색 창에서 **관리 센터**를 선택하고 프로젝트의 세부 정보 페이지를 봅니다. 이 페이지의 이미지는 다음과 같습니다.

    ![관리 센터 스크린샷](./media/ai-foundry-manage-project.png)

1. **리소스 그룹**에서 리소스 그룹을 선택하고 새 브라우저 탭의 Azure Portal에서 엽니다. 메시지가 표시되면 Azure 자격 증명으로 로그인하고 리소스 그룹 페이지를 보려면 환영 알림을 닫습니다.
...
1. 시스템 할당 ID의 상태가 **Off**이면 **On**으로 전환하고 변경 내용을 저장합니다. 그런 다음 상태의 변경이 확인될 때까지 기다립니다.
1. 리소스 그룹 페이지로 돌아가 허브에 대한 **스토리지 계정** 리소스를 선택하고 해당 **IAM(액세스 제어)** 페이지를 봅니다.

    ![Azure Portal의 스토리지 계정 액세스 제어 페이지 스크린샷.](./media/storage-access-control.png)

1. Azure AI 서비스 리소스에서 사용하는 관리 ID의`Storage blob data reader` 역할에 역할 할당을 추가합니다.
...
1. **채팅** LLM 도구 창의 **연결**에서 AI 허브의 Azure OpenAI Service 리소스에 대한 연결을 선택합니다. 그후 다음 속성을 구성합니다.
    - **API**: 채팅
...
1. **프롬프트** 필드를 다음과 같이 수정합니다.
   ...
   **Instructions**:
   ...
   ```
    ...
    추가한 프롬프트를 읽고 숙지합니다. 시스템 메시지(목표, 기능 정의 및 일부 지침 포함) 및 채팅 기록(각 사용자 질문 입력 및 각 이전 도우미 응답 출력을 표시하도록 명령)으로 구성됩니다.
...
1. 도구 모음에서 **배포**를 선택하여 다음 설정으로 흐름을 배포합니다.
    ...
    - **고급 설정**:
        - *기본 설정 사용*
1. Azure AI 파운드리 포털의 프로젝트에 있는 **내 자산** 섹션에서 **모델 + 엔드포인트** 페이지를 선택합니다.

    gpt-4o 모델에 대한 페이지가 열리면 **뒤로** 단추를 사용하여 모든 모델 및 엔드포인트를 봅니다.

1. 처음에는 페이지에 모델 배포만 표시될 수 있습니다. 배포가 나열되고 성공적으로 만들어질 때까지 다소 시간이 걸릴 수 있습니다.
1. 배포가 *성공*하면 선택합니다. 그런 다음 해당 **테스트** 페이지를 봅니다.

    > **팁**: 테스트 페이지에서 엔드포인트를 비정상이라고 설명하는 경우 **모델 및 엔드포인트**로 돌아가서 1분 정도 기다린 후 보기를 새로 고치고 엔드포인트를 다시 선택합니다.
