...
1. Azure AI Foundry 프로젝트 포털에서 왼쪽 메뉴를 사용하여 **모델 카탈로그**로 이동합니다.
    카탈로그 페이지에서** 벤치마크와 비교**를 선택합니다. 모델 벤치마크 페이지에서 다른 모델을 비교하여 차트가 이미 그려진 것을 볼 수 있습니다.
1. **+ 비교할 모델**을 선택하고 **gpt-4-32k**와 **gpt-4**를 메트릭 차트에 추가합니다. **X축** 드롭다운 메뉴에서 **품질** 아래에 있는 다음 메트릭을 선택하고 다음으로 전환하기 전에 각 결과 차트를 관찰합니다.
    - 일관성
    - 유창성
    - 접지
1. 결과를 탐색할 때 다음 질문에 대답할 수 있습니다.
    ...
    - GPT-4 32k 변형은 베이스 모델과 어떻게 다릅니까?
