---
lab:
  title: 자체 데이터를 사용하는 사용자 지정 Copilot 만들기
---

# 자체 데이터를 사용하는 사용자 지정 Copilot 만들기

RAG(검색 증강 생성)은 사용자 지정 데이터 소스의 데이터를 생성형 AI 모델에 대한 프롬프트에 통합하는 애플리케이션을 빌드하는 데 사용되는 기술입니다. RAG는 언어 모델을 사용하여 입력을 해석하고 적절한 응답을 생성하는 채팅 기반 애플리케이션인 사용자 지정 *Copilot*을 개발하는 데 일반적으로 사용되는 패턴입니다.

이 연습에서는 Azure AI 스튜디오를 사용하여 사용자 지정 데이터를 생성형 AI 프롬프트 흐름에 통합합니다.
...
Copilot 솔루션은 사용자 지정 데이터를 프롬프트 흐름에 통합합니다. 이 통합을 지원하려면 데이터를 인덱싱할 Azure AI 검색 리소스가 필요합니다.

1. 웹 브라우저에서 [Azure Portal](https://portal.azure.com)(`https://portal.azure.com`)을 열고 Azure 자격 증명을 사용하여 로그인합니다.
1. 홈 페이지에서 **+ 리소스 만들기**를 선택하고 `Azure AI Search`를 검색합니다. 그런 다음, 다음 설정을 사용하여 새 Azure AI 검색 리소스를 만듭니다.

    - **구독**: *Azure 구독 선택*
    - **리소스 그룹**: *리소스 그룹 선택 또는 만들기*
    - **서비스 이름**: *고유한 서비스 이름 입력*
    ...
    > \* 나중에 Azure AI 검색 리소스와 동일한 지역에 Azure AI 허브(Azure OpenAI Service 포함)를 만들게 됩니다. Azure OpenAI 리소스는 지역 할당량에 따라 테넌트 수준에서 제한됩니다. 나열된 지역에는 이 연습에 사용된 모델 형식에 대한 기본 할당량이 포함되어 있습니다. 지역을 임의로 선택하면 다른 사용자와 테넌트를 공유하는 시나리오에서 단일 지역이 할당량 한도에 도달할 위험이 줄어듭니다. 연습 후반에 할당량 한도에 도달하는 경우 다른 지역에 다른 Azure AI 허브를 만들어야 할 수도 있습니다.

1. Azure AI 검색 리소스 배포가 완료될 때까지 기다립니다.

## Azure AI 프로젝트 만들기

이제 Azure AI 스튜디오 프로젝트와 이를 지원하기 위한 Azure AI 리소스를 만들 준비가 되었습니다.

1. 웹 브라우저에서 `https://ai.azure.com`에서 [Azure AI 스튜디오](https://ai.azure.com)를 열고 Azure 자격 증명을 사용하여 로그인합니다.
1. Azure AI 스튜디오 **홈** 페이지에서 **+ 새 프로젝트**를 선택합니다. 그런 다음 **프로젝트 만들기** 마법사에서 다음 설정을 사용하여 프로젝트를 만듭니다.

    - **프로젝트 이름**: *프로젝트의 고유한 이름*
    - **허브**: *다음 설정으로 새 리소스를 만듭니다.*

        - **허브 이름**: *고유 이름*
        - **Azure 구독**: *사용자의 Azure 구독*.
        - **리소스 그룹**: *Azure AI 검색 리소스가 포함된 리소스 그룹을 선택합니다*.
        - **위치**: *Azure AI 검색 리소스와 동일한 위치*
        - **Azure OpenAI**: (신규) *선택한 허브 이름으로 자동 채우기*
        - **Azure AI 검색**: *Azure AI 검색 리소스 선택*
...
1. Azure AI 스튜디오 내 프로젝트 왼쪽 탐색 창의 **구성 요소**에서 **배포** 페이지를 선택합니다.
1. 다음 설정을 사용하여 **text-embedding-ada-002** 모델의 새 배포를 만듭니다.
    ...
    - **고급 옵션**:
        - **콘텐츠 필터**: *기본값*
        - **분당 토큰 속도 제한**: `5K`
1. 이전 단계를 반복하여 배포 이름 `gpt-35-turbo-16k`로 **gpt-35-turbo-16k** 모델을 배포합니다.

    > **참고**: TPM(분당 토큰)을 줄이면 사용 중인 구독에서 사용 가능한 할당량을 과도하게 사용하지 않을 수 있습니다. 이 연습에 사용된 데이터는 5,000TPM이면 충분합니다.

## 프로젝트에 데이터 추가

Copilot 데이터는 가상의 여행사 *Margie's Travel*의 PDF 형식으로 된 여행 브로 세트로 구성됩니다. 프로젝트에 이를 추가해 보겠습니다.

1. `https://github.com/MicrosoftLearning/mslearn-ai-studio/raw/main/data/brochures.zip`에서 [ 브로슈어 압축 아카이브](https://github.com/MicrosoftLearning/mslearn-ai-studio/raw/main/data/brochures.zip)를 다운로드하여 로컬 파일 시스템의 **브로슈어**라는 폴더에 압축을 풉니다.
1. Azure AI 스튜디오의 프로젝트에서 왼쪽 탐색 창의 **구성 요소** 아래 **데이터** 페이지를 선택합니다.
1. **+ 새 데이터**를 선택합니다.
1. **데이터 추가** 마법사에서 드롭다운 메뉴를 확장하여 **파일/폴더 업로드**를 선택합니다.
1. **폴더 업로드**를 선택하고 **브로** 폴더를 선택합니다.
1. 데이터 이름을 `brochures`로 설정합니다.
1. 폴더가 업로드될 때까지 기다렸다가 폴더에 여러 개의 .pdf 파일이 포함되어 있는지 확인합니다.

## 데이터에 대한 인덱스 만들기

이제 프로젝트에 데이터 원본을 추가했으므로 이 데이터 원본을 사용하여 Azure AI 검색 리소스에서 인덱스를 만들 수 있습니다.

1. Azure AI 스튜디오의 프로젝트에서 왼쪽 탐색 창의 **구성 요소** 아래 **인덱스** 페이지를 선택합니다.
1. 다음 설정을 사용하여 새 인덱스를 추가합니다.
    - **원본 데이터**:
        - **데이터 원본**: Azure AI 스튜디오의 데이터
            - ***브로** 데이터 원본을 선택합니다*.
    - **인덱스 설정**:
        - **Azure AI 검색 서비스 선택**: *Azure AI 검색 리소스에 대한 **AzureAISearch** 연결을 선택합니다*.
        - **인덱스 이름**: `brochures-index`
        - **가상 머신**: 자동 선택
    - **검색 설정**:
        - **벡터 설정**: 이 검색 리소스에 벡터 검색을 추가합니다.
        - **임베딩 모델 선택**: *허브의 기본 Azure OpenAI 리소스를 선택합니다.*
...
1. 왼쪽 탐색 창의 **프로젝트 플레이그라운드**에서 **채팅** 페이지를 선택합니다.
1. 채팅 페이지의 옵션 패널에서 **gpt-35-turbo-16k** 모델 배포가 선택되어 있는지 확인합니다. 그런 다음 기본 채팅 세션 패널에서 프롬프트 `Where can I stay in New York?`를 제출합니다.
1. 응답을 검토합니다. 이때 응답은 인덱스의 데이터 없이 모델에서 생성된 일반적인 답변이어야 합니다.
1. 설정 패널에서 **데이터 추가** 탭을 선택한 다음 **브로슈어-인덱스** 프로젝트 인덱스를 추가하고 **하이브리드(벡터+키워드)** 검색 유형을 선택합니다.
...
벡터 인덱스가 Azure AI 스튜디오 프로젝트에 저장되어 프롬프트 흐름에서 쉽게 사용할 수 있습니다.

1. Azure AI 스튜디오의 프로젝트에서 왼쪽 탐색 창의 **도구** 아래 **프롬프트 흐름** 페이지를 선택합니다.
...
> **참고**: 지역 및 데이터 센터 로드에 따라 배포에 시간이 걸릴 수 있습니다. 배포하는 동안 아래의 챌린지 섹션으로 이동하거나 시간이 짧은 경우 배포 테스트를 건너뜁니다.
...
이제 Azure AI 스튜디오로 빌드된 Copilot에서 자체 데이터를 통합하는 방법을 경험해 보셨으니, 더 자세히 살펴보도록 하겠습니다!

Azure AI 스튜디오를 통해 새 데이터 원본을 추가하고, 인덱싱하고, 인덱싱된 데이터를 프롬프트 흐름에 통합해 보세요. 시도할 수 있는 일부 데이터 세트는 다음과 같습니다.
...
1. Azure AI 스튜디오 탐색을 마친 경우 [Azure Portal](https://portal.azure.com)의 `https://portal.azure.com`로 돌아가서 필요한 경우 Azure 자격 증명을 사용하여 로그인합니다. 그런 다음 Azure AI 검색 및 Azure AI 리소스를 프로비전한 리소스 그룹의 리소스를 삭제합니다.
