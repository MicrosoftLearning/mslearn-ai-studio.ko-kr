...
1. 구독에서 사용 가능한 컴퓨팅 리소스에 따라 시간이 걸릴 수 있는 인덱싱 프로세스가 완료될 때까지 기다립니다. 인덱스 만들기 작업은 다음 작업으로 구성됩니다.
...
Azure AI 파운드리 포털을 통해 새 데이터 원본을 추가하고, 인덱싱하고, 인덱싱한 데이터를 클라이언트 앱에 통합합니다. 시도할 수 있는 일부 데이터 세트는 다음과 같습니다.
...
선택한 데이터 집합에서만 응답할 수 있는 프롬프트를 제출하여 솔루션을 테스트합니다.
