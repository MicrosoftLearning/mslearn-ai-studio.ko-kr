...
1. **모델 + 엔드포인트** 페이지로 돌아가 이전 단계를 반복하여 TPM 속도 제한이 **50K**(50K 미만인 경우 해당 구독에서 사용할 수 있는 최대값)인 최신 버전의 **글로벌 표준** 배포로 **gpt-4o** 모델을 배포합니다.
...
1. 아래 코드에서 다음 자리 표시자를 바꿉니다. 
    ...
    - **your_model_deployment** **gpt-4o** 모델 배포에 할당한 이름(반드시 이 되어야 함)으로 바꿉니다
