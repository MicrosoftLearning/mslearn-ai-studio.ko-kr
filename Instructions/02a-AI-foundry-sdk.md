...
1. **만들기**를 선택하고 선택한 Gpt-4o 모델 배포를 포함한 프로젝트가 생성될 때까지 기다립니다.

    > <font color="red"><b>중요</b>:</font> gpt-4o 모델에 사용 가능한 할당량에 따라 다른 지역의 리소스에 모델을 배포하라는 추가 프롬프트가 표시될 수 있습니다. 이 경우 기본 설정을 사용하여 수행합니다. 연습 후반부에서는 기본 프로젝트 엔드포인트를 사용할 수 <b><u>없습니다</u></b>. 모델별 대상 URI를 사용해야 합니다.

1. 프로젝트를 만들면 모델을 테스트할 수 있도록 채팅 플레이그라운드가 자동으로 열립니다(그렇지 않은 경우 왼쪽 작업 창에서 **플레이그라운드**를 선택한 다음 **채팅 플레이그라운드**를 엽니다).
...
1. 제공된 구성 파일을 편집하려면 다음 명령을 입력합니다.
   ...
   code .env
    ...
    > <font color="red"><b>중요</b>:</font> 프로젝트의 기본 지역에 gpt-4o 모델을 배포한 경우 프로젝트의 개요<b> 페이지에서 Azure AI Foundry 프로젝트</b> 또는 <b>Azure OpenAI</b> 엔드포인트<b>를 사용하여 </b>모델에 연결할 수 있습니다. 할당량이 부족하여 모델을 다른 지역에 배포한 경우 <b>모델 + 엔드포인트</b> 페이지에서 모델을 선택하고 모델의 <b>대상 URI</b>을 사용합니다.

1. 코드 파일에서 **your_project_endpoint** 자리 표시자를 모델에 적합한 엔드포인트로 바꾸고, **your_model_deployment** 자리 표시자를 Gpt-4o 모델 배포에 할당한 이름으로 바꿉니다.
